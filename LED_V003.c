// コマンド
//  gcc -o LED_V003 LED_V003.c -lwiringPi -lpthread -g -Wall 
...
// パネルのサイズ定義
const unsigned int display_width = 192;
...
const unsigned long pattern[16] = {
    ...
    0x00000000,                                                         /*2*/
    0x00000000,                                                         /*3*/
    0b1001001001001001001001001001001001001001001001001001001001001001, /*4*/
    0b1001001001001001001001001001001001001001001001001001001001001001, /*5*/
    0b1001001001001001001001001001001001001001001001001001001001001001, /*6*/
    0b1001001001001001001001001001001001001001001001001001001001001001, /*7*/
...
void calloc_check_uint(unsigned int **, size_t);
...
int main(void)
{
    ...
    // datalook(maindata.file_data, maindata.size,(size_t) 5*2);//デバッグ用
    ...
    pthread_create(&t[1], NULL, LED_drive_B, NULL);

    display_image_write(decodedata, 0, 0, (size_t)display_width * display_height);
    while (1)
    {
    }
}
...
void file_load(image *maindata)
{
    ...
    calloc_check_uint(&maindata->width, sizeof(unsigned int) * maindata->seets);
    calloc_check_uint(&maindata->height, sizeof(unsigned int) * maindata->seets);
...
void calloc_check_uint(unsigned int **point, size_t size)
{
    // size=確保するメモリサイズ
    ...
    while (1)
    {
        for (multi_finish.count = 0; multi_finish.count < 64; ++multi_finish.count)
        {
            for (multi_finish.y = 0; multi_finish.y < (display_height / 2); ++multi_finish.y)
            {
                ...
                {
                    multi_finish.finish_1A = 1; // driveBに指令
                    ...
                    while (multi_finish.finish_1A) // 相手を待つ
                    {
                    }
...
void *LED_drive_B(void *data)
{
    while (1)
    {
        while (multi_finish.finish_1A == 0)
        {
            // 指令待ち
        }
        ...
        multi_finish.finish_1A = 0; // LEDの処理が終わったことを通知
    }
}
